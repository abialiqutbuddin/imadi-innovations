...
export default function VisionSlide({ disableAnimations = false }: { disableAnimations?: boolean }) {
    return (
        <div className="w-full h-full flex flex-col justify-center items-center bg-transparent px-6 md:px-20">
            <motion.h2
                ...
                className="text-4xl md:text-6xl font-bold font-outfit text-foreground mb-16"
            >
                Our Vision
            </motion.h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl w-full">
                {visionPoints.map((point, index) => (
                    <motion.div
                        ...
                        className="bg-white/50 dark:bg-brand-navy/30 p-8 rounded-2xl border border-gray-200 dark:border-white/5 hover:border-brand-orange/50 transition-colors"
                    >
                        <div className="mb-6 bg-black/5 dark:bg-white/5 w-16 h-16 rounded-full flex items-center justify-center">
                            {point.icon}
                        </div>
                        <h3 className="text-2xl font-bold text-foreground mb-4 font-outfit">{point.title}</h3>
